...
if [ $MF_RUN_OPT  = 'RUN' ]; then
    
     export INDATE=20230302
     export ODATE=${INDATE:2}
      
     mfbsijcl $PROGNAME PARMS JOB=MLC ODATE=$ODATE END-PARMS  >>$logfile
     rc=$?
 else
     mfbsijcl /jrestart:$MF_RESTART_OPTION PARMS JOB=MLC MODE=BATCH END-PARMS  >>$logfile 
     rc=$?
 fi 
